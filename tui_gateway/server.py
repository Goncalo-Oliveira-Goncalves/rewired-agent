...
_rewired_home = get_rewired_home()
load_rewired_dotenv(
    rewired_home=_rewired_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.rewired/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_rewired_home, "logs", "tui_gateway_crash.log")
...
_cfg_path = None
try:
    _slash_timeout = float(os.environ.get("REWIRED_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("REWIRED_TUI_RPC_POOL_WORKERS") or "4")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent RewiredCLI subprocess for slash commands."""
...
def _notify_session_boundary(event_type: str, session_id: str | None) -> None:
    ...
    try:
        from rewired_cli.plugins import invoke_hook as _invoke_hook
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `rewired` shows the prompt before constructing AIAgent; the TUI used
...
def _save_cfg(cfg: dict):
    ...
    path = _rewired_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["REWIRED_GATEWAY_SESSION"] = "1"
    os.environ["REWIRED_EXEC_ASK"] = "1"
    os.environ["REWIRED_INTERACTIVE"] = "1"
...
def resolve_skin() -> dict:
    try:
        from rewired_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("REWIRED_MODEL", "")
        or os.environ.get("REWIRED_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("REWIRED_TUI_PROVIDER", "").strip()
    ...
    try:
        from rewired_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("REWIRED_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _load_reasoning_config() -> dict | None:
    from rewired_constants import parse_reasoning_effort
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("REWIRED_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("REWIRED_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] REWIRED_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from rewired_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown REWIRED_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in REWIRED_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid REWIRED_TUI_TOOLSETS entries; using configured CLI toolsets"
        )

    try:
        from rewired_cli.config import load_config
        from rewired_cli.tools_config import _get_platform_tools
        ...
        if fallback_notice is not None:
            print(
                "[tui] no valid REWIRED_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _persist_model_switch(result) -> None:
    from rewired_cli.config import save_config
...
def _apply_model_switch(sid: str, session: dict, raw_input: str) -> dict:
    from rewired_cli.model_switch import parse_model_flags, switch_model
    from rewired_cli.runtime_provider import resolve_runtime_provider
    ...
    custom_provs = None
    try:
        from rewired_cli.config import get_compatible_custom_providers, load_config
    ...
    os.environ["REWIRED_MODEL"] = result.new_model
    os.environ["REWIRED_INFERENCE_MODEL"] = result.new_model
    ...
    # in config or env.
    #
    # REWIRED_TUI_PROVIDER is the canonical "explicit-this-process" carrier
    ...
    if result.target_provider:
        os.environ["REWIRED_INFERENCE_PROVIDER"] = result.target_provider
        os.environ["REWIRED_TUI_PROVIDER"] = result.target_provider
    if persist_global:
...
def _session_info(agent) -> dict:
    ...
    try:
        ...
        pass
    try:
        from rewired_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from rewired_cli.banner import get_update_result
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from rewired_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from rewired_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("REWIRED_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("REWIRED_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _make_agent(sid: str, key: str, session_id: str | None = None):
    ...
    return AIAgent(
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("REWIRED_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("REWIRED_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("REWIRED_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("REWIRED_IGNORE_RULES")),
        **_agent_cbs(sid),
    )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `REWIRED_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    filename = os.path.abspath(
        f"rewired_conversation_{_time.strftime('%Y%m%d_%H%M%S')}.json"
    )
    try:
...
#
# Layout:  $REWIRED_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_rewired_home() / "spawn-trees"
        ...
        try:
            ...
            if images:
                try:
                    ...
                    from rewired_cli.config import load_config as _tui_load_config
            ...
            if status == "complete" and isinstance(raw, str) and raw.strip():
                try:
                    from rewired_cli.goals import GoalManager
            ...
            if (
                ...
                try:
                    from rewired_cli.voice import speak_text
                    ...
                    logger.warning("voice TTS skipped: rewired_cli.voice unavailable")
                except Exception as e:
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from rewired_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _rewired_home / "images"
...
def _(rid, params: dict) -> dict:
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from rewired_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        try:
            if session:
                ...
                current = is_truthy_value(os.environ.get("REWIRED_YOLO_MODE"))
                if current:
                    os.environ.pop("REWIRED_YOLO_MODE", None)
                    nv = "0"
                else:
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from rewired_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_rewired_home), "display": display_rewired_home()})
    if key == "full":
    ...
    if key == "mouse":
        ...
        return _ok(rid, {"value": "on" if on else "off"})
    if key == "mtime":
        cfg_path = _rewired_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from rewired_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if not user_confirm:
            try:
                from rewired_cli.config import load_config as _load_config
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.rewired/.env`` into the gateway process via
    ``rewired_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from rewired_cli.config import reload_env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from rewired_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `rewired` is interactive — use `/rewired chat -q …` or run `rewired` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`rewired setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`rewired gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`rewired sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`rewired config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m rewired_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "rewired_cli.main", *argv],
            capture_output=True,
            text=True,
...
def _(rid, params: dict) -> dict:
    try:
        from rewired_cli.commands import resolve_command
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from rewired_cli.plugins import (
...
def _(rid, params: dict) -> dict:
    ...
    paste_dir = _rewired_home / "pastes"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from rewired_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    try:
        ...
        # "models" with the curated list (same source as `rewired model` and
        ...
        from rewired_cli.auth import PROVIDER_REGISTRY as _auth_reg
        ...
        for entry in CANONICAL_PROVIDERS:
            if entry.slug in authed_map:
                ...
                if auth_type == "api_key" and key_env:
                    ...
                    warning = f"run `rewired model` to configure ({auth_type})"
                ordered.append(
                    {
                        "slug": entry.slug,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from rewired_cli.config import is_managed, save_env_value
        from rewired_cli.model_switch import list_authenticated_providers
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `rewired model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.rewired/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from rewired_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from rewired_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("REWIRED_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("REWIRED_VOICE_TTS", "").strip() == "1"
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in ("on", "off"):
        ...
        os.environ["REWIRED_VOICE"] = "1" if enabled else "0"

        if not enabled:
            # Disabling the mode must tear the continuous loop down; the
            # loop holds the microphone and would otherwise keep running.
            try:
                from rewired_cli.voice import stop_continuous
    ...
    if action == "tts":
        ...
        os.environ["REWIRED_VOICE_TTS"] = "1" if new_value else "0"
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            from rewired_cli.voice import start_continuous
...
@method("voice.remote")
def _(rid, params: dict) -> dict:
    """Start a remote mic relay: serves a WebSocket mic page, returns the URL."""
    try:
        import asyncio
        import io
        import tempfile
            ...
            async for msg in ws:
                if msg.type == web.WSMsgType.TEXT:
                    ...
                    if data == "start":
                        ...
                        threading.Thread(target=_transcribe_remote_audio, args=(list(audio_chunks), session_id), daemon=True).start()
                    elif data == "ping":
...
<title>Rewired Voice</title>
<style>
* { margin: 0; padding: 0; box-sizing: border-box; }
body {
  font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', system-ui, sans-serif;
  background: #1a1a24; color: #e0e0e0; display: flex;
  justify-content: center; align-items: center; min-height: 100vh;
}
.card {
  background: #252535; border-radius: 16px; padding: 40px;
  text-align: center; max-width: 400px; box-shadow: 0 8px 32px rgba(0,0,0,0.4);
}
...
.btn-record.recording { background: #bdc3c7; color: #666; }
.btn-stop { background: #3498db; color: #fff; }
.btn-stop:hover { background: #2980b9; }
.status { margin-top: 20px; font-size: 13px; color: #666; }
.wave { margin-top: 24px; height: 40px; display: flex; justify-content: center; gap: 3px; }
.wave span {
  display: inline-block; width: 4px; height: 100%;
  background: #3498db; border-radius: 2px;
  animation: none; transform: scaleY(0.2); transition: transform 0.1s;
}
.wave.active span { animation: wave 0.8s ease-in-out infinite; }
...
<div class="card">
  <h1>🎤 Rewired Voice</h1>
  <p>Microphone access is needed for voice input to the remote Rewired agent.</p>
  <button class="btn btn-record" id="btn" onclick="toggleMic()">Start Recording</button>
...
function connect() {
  ...
  ws.onopen = () => status.textContent = 'Connected. Click to record.';
  ws.onmessage = (e) => {
    if (e.data === 'ok') status.textContent = 'Recording... Speak now.';
  };
  ws.onclose = () => {
...
async function toggleMic() {
  ...
  try {
    ...
    ws.send('start');
    mediaRecorder.ondataavailable = (e) => { if (e.data.size > 0) ws.send(await e.data.arrayBuffer()); };
    mediaRecorder.start(100);
    isRecording = true;
    btn.textContent = 'Stop Recording';
    btn.className = 'btn btn-stop';
    wave.className = 'wave active';
...
connect();
        ...
        async def transcribe_handler(request):
            """Accept raw WAV POST, transcribe, return text."""
            import tempfile
            ...
            try:
                ...
                result = transcribe_recording(tmp_path)
                os.unlink(tmp_path)
...
def _transcribe_remote_audio(chunks: list[bytes], session_id: str | None = None) -> None:
    """Transcribe remote audio chunks and emit voice.transcript event."""
    import tempfile
    import wave
    import io
    ...
    # Write chunks to a temp WAV file
    try:
        ...
        logger.error("remote voice transcription failed: %s", e)
        _voice_emit("voice.transcript", {"text": "[transcription error]"})


def _get_host_ip() -> str:
    """Get a non-loopback IP address for the remote voice URL."""
    try:
        import socket
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from rewired_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from rewired_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from rewired_cli.browser_connect import try_launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        from rewired_cli.plugins import get_plugin_manager
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("REWIRED_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("REWIRED_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_rewired_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from rewired_cli.tools_config import (
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from rewired_cli.skills_hub import do_install
        ...
        if action == "browse":
            from rewired_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from rewired_cli.skills_hub import inspect_skill
