...
WEB_DIST = Path(os.environ["REWIRED_WEB_DIST"]) if "REWIRED_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
_log = logging.getLogger(__name__)

app = FastAPI(title="Rewired", version=__version__)
...
# or REWIRED_DASHBOARD_TUI=1.  Set from :func:`start_server`.
...
async def get_status():
    ...
    return {
        ...
        "hermes_home": str(get_rewired_home()),
...
# streamed to a per-action log file under ``~/.rewired/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_rewired_home() / "logs"
...
def _spawn_hermes_action(subcommand: List[str], name: str) -> subprocess.Popen:
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "REWIRED_NONINTERACTIVE": "1"},
    }
...
async def set_model_assignment(body: ModelAssignment):
    ...
    Writes to ``~/.rewired/config.yaml`` — applies to **new** sessions only.
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.rewired/.anthropic_oauth.json`` — Hermes-managed PKCE flow
    ...
    try:
        ...
        _REWIRED_OAUTH_FILE = None  # type: ignore
    ...
    if hermes_creds and hermes_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hermes_pkce",
            "source_label": f"Hermes PKCE ({_REWIRED_OAUTH_FILE})",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from rewired_cli import auth as hauth
        if provider_id == "nous":
...
async def disconnect_oauth_provider(provider_id: str, request: Request):
    ...
    if provider_id in ("anthropic", "claude-code"):
        try:
            from agent.anthropic_adapter import _REWIRED_OAUTH_FILE
            if _REWIRED_OAUTH_FILE.exists():
                _REWIRED_OAUTH_FILE.unlink()
        except Exception:
            pass
        # Also clear the credential pool entry if present.
        try:
            from rewired_cli.auth import clear_provider_auth
...
#          → persists to ~/.rewired/.anthropic_oauth.json AND credential pool
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    ...
    _REWIRED_OAUTH_FILE.parent.mkdir(parents=True, exist_ok=True)
    _REWIRED_OAUTH_FILE.write_text(json.dumps(payload, indent=2), encoding="utf-8")
...
async def _start_device_code_flow(provider_id: str) -> Dict[str, Any]:
    ...
    """
    if provider_id == "nous":
        from rewired_cli.auth import _request_device_code, PROVIDER_REGISTRY
        import httpx
        pconfig = PROVIDER_REGISTRY["nous"]
        portal_base_url = (
            os.getenv("REWIRED_PORTAL_BASE_URL")
...
def _nous_poller(session_id: str) -> None:
    ...
    from rewired_cli.auth import _poll_for_token, refresh_nous_oauth_from_state
    ...
    try:
        ...
        from rewired_cli.auth import persist_nous_credentials
...
def _codex_full_login_worker(session_id: str) -> None:
    ...
    try:
        ...
        base_url = (
            os.getenv("REWIRED_CODEX_BASE_URL", "").strip().rstrip("/")
...
async def get_logs(
    ...
    from rewired_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_rewired_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from rewired_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _resolve_profile_dir(name: str) -> Path:
    ...
    from rewired_cli import profiles as profiles_mod
    try:
...
async def get_skills():
    ...
    from rewired_cli.skills_config import get_disabled_skills
    config = load_config()
...
async def toggle_skill(body: SkillToggle):
    from rewired_cli.skills_config import get_disabled_skills, save_disabled_skills
    config = load_config()
    disabled = get_disabled_skills(config)
    if body.enabled:
...
try:
    from rewired_cli.pty_bridge import PtyBridge, PtyUnavailableError
...
def _resolve_chat_argv(
    ...
    Spawns the Rust TUI binary (``rewired-tui``).  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build the TUI binary.

    Session resume is propagated via the ``REWIRED_TUI_RESUME`` env var —
    matching what ``rewired_cli.main._launch_tui`` does for the CLI path.
    """
    from rewired_cli.main import PROJECT_ROOT
    ...
    env.setdefault("REWIRED_TUI_DISABLE_MOUSE", "1")

    if resume:
        latest_resume, _latest_path = _session_latest_descendant(resume)
        if latest_resume:
            resume = latest_resume
        env["REWIRED_TUI_RESUME"] = resume

    if sidecar_url:
        env["REWIRED_TUI_SIDECAR_URL"] = sidecar_url
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    try:
        ...
        # _resolve_chat_argv calls sys.exit(1) when the Rust TUI binary is missing.
...
# REWIRED_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def mount_spa(application: FastAPI):
    ...
    and the SPA's runtime ``__REWIRED_BASE_PATH__`` honour that prefix
    ...
    def _serve_index(prefix: str = ""):
        ...
        token_script = (
            f'<script>window.__REWIRED_SESSION_TOKEN__="{_SESSION_TOKEN}";'
            f"window.__REWIRED_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
            f'window.__REWIRED_BASE_PATH__="{prefix}";</script>'
        )
        if prefix:
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.rewired/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.rewired/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_rewired_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.rewired/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    1. User plugins:    ~/.rewired/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hermes/plugins/  (only if REWIRED_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from rewired_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_rewired_home() / "plugins", "user"),
        (bundled_root / "memory", "bundled"),
        (bundled_root, "bundled"),
    ]
    if os.environ.get("REWIRED_ENABLE_PROJECT_PLUGINS"):
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    from rewired_cli.plugins_cmd import (
    ...
    plugins_root_resolved = (get_rewired_home() / "plugins").resolve()
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from rewired_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from rewired_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from rewired_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from rewired_cli.plugins_cmd import dashboard_remove_user_plugin
