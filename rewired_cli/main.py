#!/usr/bin/env python3
"""
Rewired CLI - Main entry point.

Usage:
    rewired                     # Interactive chat (default)
    rewired chat                # Interactive chat
    rewired gateway             # Run gateway in foreground
    rewired gateway start       # Start gateway as service
    rewired gateway stop        # Stop gateway service
    rewired gateway status      # Show gateway status
    rewired gateway install     # Install gateway service
    rewired gateway uninstall   # Uninstall gateway service
    rewired setup               # Interactive setup wizard
    rewired logout              # Clear stored authentication
    rewired status              # Show status of all components
    rewired cron                # Manage cron jobs
    rewired cron list           # List cron jobs
    rewired cron status         # Check if cron scheduler is running
    rewired doctor              # Check configuration and dependencies
    rewired honcho setup                    # Configure Honcho AI memory integration
    rewired honcho status                   # Show Honcho config and connection status
    rewired honcho sessions                 # List directory → session name mappings
    rewired honcho map <name>               # Map current directory to a session name
    rewired honcho peer                     # Show peer names and dialectic settings
    rewired honcho peer --user NAME         # Set user peer name
    rewired honcho peer --ai NAME           # Set AI peer name
    rewired honcho peer --reasoning LEVEL   # Set dialectic reasoning level
    rewired honcho mode                     # Show current memory mode
    rewired honcho mode [hybrid|honcho|local]  # Set memory mode
    rewired honcho tokens                   # Show token budget settings
    rewired honcho tokens --context N       # Set session.context() token cap
    rewired honcho tokens --dialectic N     # Set dialectic result char cap
    rewired honcho identity                 # Show AI peer identity representation
    rewired honcho identity <file>          # Seed AI peer identity from a file (SOUL.md etc.)
    rewired honcho migrate                  # Step-by-step migration guide: OpenClaw native → Rewired + Honcho
    rewired version             Show version
    rewired update              Update to latest version
    rewired uninstall           Uninstall Rewired
    rewired acp                 Run as an ACP server for editor integration
    rewired sessions browse     Interactive session picker with search

    rewired claw migrate --dry-run  # Preview migration without changes
"""
...
# When the user upgrades code via ``git pull`` (or ``rewired update``
...
# ``rewired update`` to recover.  Missing the bootstrap means UTF-8 stdio
# setup is skipped on Windows — degraded, not broken.  POSIX is unaffected.
try:
    import rewired_bootstrap  # noqa: F401
...
def _add_accept_hooks_flag(parser) -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "(equivalent to REWIRED_ACCEPT_HOOKS=1 / hooks_auto_accept: true)."
        ),
    )
...
def _require_tty(command_name: str) -> None:
    ...
    Interactive TUI commands (hermes tools, rewired setup, rewired model) use
    curses or input() prompts that spin at 100% CPU when stdin is a pipe.
    This guard prevents accidental non-interactive invocation.
    """
    if not sys.stdin.isatty():
        print(
            f"Error: 'rewired {command_name}' requires an interactive terminal.\n"
...
#
# Many modules cache REWIRED_HOME at import time (module-level constants).
# We intercept --profile/-p from sys.argv here and set the env var so that
# every subsequent ``os.getenv("REWIRED_HOME", ...)`` resolves correctly.
# The flag is stripped from sys.argv so argparse never sees it.
# Falls back to ~/.rewired/active_profile for sticky default.
# ---------------------------------------------------------------------------
def _apply_profile_override() -> None:
    """Pre-parse --profile/-p and set REWIRED_HOME before module imports."""
    argv = sys.argv[1:]
    profile_name = None
    consume = 0
    ...
    # 1.5 If REWIRED_HOME is already set and no explicit flag was given, trust it
    # only when it already points to a specific profile directory.  The
    # distinguishing heuristic: a profile path has "profiles" as its immediate
    # parent directory name (e.g. ~/.rewired/profiles/coder or
    # /opt/data/profiles/coder).  If REWIRED_HOME points to the hermes root
    # instead (e.g. systemd hardcodes REWIRED_HOME=/root/.hermes), we must
    # still read active_profile — the user may have switched profiles via
    # `hermes profile use` and the gateway should honour that choice.
    # See issue #22502.
    hermes_home_env = os.environ.get("REWIRED_HOME", "")
    ...
    if profile_name is None:
        try:
            from rewired_constants import get_default_rewired_root

            active_path = get_default_rewired_root() / "active_profile"
    ...
    # 3. If we found a profile, resolve and set REWIRED_HOME
    if profile_name is not None:
        try:
            from rewired_cli.profiles import resolve_profile_env
            ...
            return
        os.environ["REWIRED_HOME"] = hermes_home
...
# Load .env from ~/.rewired/.env first, then project root as dev fallback.
# User-managed env files should override stale shell exports on restart.
from rewired_cli.config import get_rewired_home
from rewired_cli.env_loader import load_rewired_dotenv

load_rewired_dotenv(project_env=PROJECT_ROOT / ".env")

# Bridge security.redact_secrets from config.yaml → REWIRED_REDACT_SECRETS env
# var BEFORE hermes_logging imports agent.redact (which snapshots the flag at
# module-import time). Without this, config.yaml's toggle is ignored because
# the setup_logging() call below imports agent.redact, which reads the env var
# exactly once. Env var in .env still wins — this is config.yaml fallback only.
try:
    if "REWIRED_REDACT_SECRETS" not in os.environ:
        import yaml as _yaml_early

        _cfg_path = get_rewired_home() / "config.yaml"
        if _cfg_path.exists():
            ...
            if isinstance(_early_sec_cfg, dict):
                ...
                if _early_redact is not None:
                    os.environ["REWIRED_REDACT_SECRETS"] = str(_early_redact).lower()
            del _early_sec_cfg
        del _cfg_path
except Exception:
...
try:
    from rewired_logging import setup_logging as _setup_logging
...
try:
    from rewired_cli.config import load_config as _load_config_early
    from rewired_constants import apply_ipv4_preference as _apply_ipv4
...
from rewired_cli import __version__, __release_date__
from rewired_constants import AI_GATEWAY_BASE_URL, OPENROUTER_BASE_URL
...
def _has_any_provider_configured() -> bool:
    ...
    from rewired_cli.config import get_env_path, get_rewired_home, load_config
    from rewired_cli.auth import get_auth_status

    # Determine whether Hermes itself has been explicitly configured (model
    # in config that isn't the hardcoded default). Used below to gate external
    # tool credentials (Claude Code, Codex CLI) that shouldn't silently skip
    # the setup wizard on a fresh install.
    from rewired_cli.config import DEFAULT_CONFIG
    ...
    from rewired_cli.auth import PROVIDER_REGISTRY
    ...
    auth_file = get_rewired_home() / "auth.json"
    if auth_file.exists():
        try:
            import json
...
def _resolve_last_session(source: str = "cli") -> Optional[str]:
    ...
    db = None
    try:
        from rewired_state import SessionDB
...
def _print_tui_exit_summary(
    ...
    print(f"  rewired --resume {target}")
    if title:
        print(f'  rewired -c "{title}"')
    print()
...
def _launch_tui(
    ...
    active_session_fd, active_session_file = tempfile.mkstemp(
        prefix="rewired-tui-active-session-", suffix=".json"
    )
    os.close(active_session_fd)
    env["REWIRED_TUI_ACTIVE_SESSION_FILE"] = active_session_file

    if model:
        env["REWIRED_MODEL"] = model
        env["REWIRED_INFERENCE_MODEL"] = model
    if provider:
        env["REWIRED_TUI_PROVIDER"] = provider
    if query:
        env["REWIRED_TUI_QUERY"] = query
    if resume_session_id:
        env["REWIRED_TUI_RESUME"] = resume_session_id

    tui_bin = PROJECT_ROOT / "tui-rs" / "target" / "release" / "rewired-tui"
    if not tui_bin.exists():
        tui_bin = PROJECT_ROOT / "tui-rs" / "target" / "debug" / "rewired-tui"

    code: Optional[int] = None
    try:
        try:
            code = subprocess.call([str(tui_bin)], env=env)
...
def _pin_kanban_board_env() -> None:
    """Pin the active kanban board into ``REWIRED_KANBAN_BOARD`` for the chat session.
    ...
    """
    if os.environ.get("REWIRED_KANBAN_BOARD"):
        return
    try:
        from rewired_cli.kanban_db import get_current_board

        os.environ["REWIRED_KANBAN_BOARD"] = get_current_board()
    except Exception:
        pass
...
def cmd_chat(args):
    ...
    use_tui = not getattr(args, "classic", False)
    ...
    if not _has_any_provider_configured():
        ...
        print()
        print("  Run:  rewired setup")
        print()

        from rewired_cli.setup import (
        ...
        if reply in ("", "y", "yes"):
            ...
            return
        print()
        print("You can run 'rewired setup' at any time to configure.")
        sys.exit(1)

    # Start update check in background (runs while other init happens)
    try:
        from rewired_cli.banner import prefetch_update_check
    ...
    if getattr(args, "yolo", False):
        os.environ["REWIRED_YOLO_MODE"] = "1"

    # --ignore-user-config: make load_cli_config() / load_config() skip the
    # user's ~/.rewired/config.yaml and return built-in defaults. Set BEFORE
    # importing cli (which runs `CLI_CONFIG = load_cli_config()` at module
    # import time). Credentials in .env are still loaded — this flag only
    # ignores behavioral/config settings.
    if getattr(args, "ignore_user_config", False):
        os.environ["REWIRED_IGNORE_USER_CONFIG"] = "1"

    # --ignore-rules: skip auto-injection of AGENTS.md/SOUL.md/.cursorrules
    # (rules), memory entries, and any preloaded skills coming from user config.
    # Maps to AIAgent(skip_context_files=True, skip_memory=True).
    if getattr(args, "ignore_rules", False):
        os.environ["REWIRED_IGNORE_RULES"] = "1"

    # --source: tag session source for filtering (e.g. 'tool' for third-party integrations)
    if getattr(args, "source", None):
        os.environ["REWIRED_SESSION_SOURCE"] = args.source
...
def cmd_gateway(args):
    ...
    from rewired_cli.gateway import gateway_command
...
def cmd_whatsapp(args):
    ...
    from rewired_cli.config import get_env_value, save_env_value
    ...
    session_dir = get_rewired_home() / "whatsapp" / "session"
    ...
    if (session_dir / "creds.json").exists():
        ...
        if response.lower() in ("y", "yes"):
            ...
            print("  Start the gateway with: rewired gateway")
            return
    ...
    if (session_dir / "creds.json").exists():
        ...
        if wa_mode == "bot":
            print("  Next steps:")
            print("    1. Start the gateway:  rewired gateway")
            print("    2. Send a message to the bot's WhatsApp number")
            print("    3. The agent will reply automatically")
            print()
            print("  Tip: Agent responses are prefixed with '⚕ Rewired'")
        else:
            print("  Next steps:")
            print("    1. Start the gateway:  rewired gateway")
        ...
        print()
        print("  Or install as a service: rewired gateway install")
    else:
...
def cmd_setup(args):
    ...
    from rewired_cli.setup import run_setup_wizard
...
def select_provider_and_model(args=None):
    ...
    Shared by ``cmd_model`` (``rewired model``) and the setup wizard
    (``setup_model_provider`` in setup.py).  Handles the full flow:
    provider picker, credential prompting, model selection, and config
    persistence.
    """
    from rewired_cli.auth import (
        resolve_provider,
        AuthError,
        format_auth_error,
    )
    from rewired_cli.config import (
        get_compatible_custom_providers,
        load_config,
        get_env_value,
    )
    from rewired_cli.providers import resolve_provider_full
    ...
    effective_provider = (
        config_provider or os.getenv("REWIRED_INFERENCE_PROVIDER") or "auto"
    )
    ...
    if effective_provider != "auto":
        ...
        if active_def is not None:
            ...
            warning = (
                f"Unknown provider '{effective_provider}'. Check 'rewired model' for "
                "available providers, or run 'rewired doctor' to diagnose config "
                "issues."
            )
    ...
    from rewired_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    def _named_custom_provider_map(cfg) -> dict[str, dict[str, str]]:
        from rewired_cli.config import read_raw_config
    ...
    # a leftover OPENAI_BASE_URL in ~/.rewired/.env can poison auxiliary
...
def _clear_stale_openai_base_url():
    """Remove OPENAI_BASE_URL from ~/.rewired/.env if the active provider is not 'custom'.
    ...
    """
    from rewired_cli.config import get_env_value, save_env_value, load_config
...
# `rewired model` provider picker. It does NOT re-run credential setup — it
# only routes already-authenticated providers to specific aux tasks. Users
# configure new providers through the normal `rewired model` flow first.
# ─────────────────────────────────────────────────────────────────────────────
...
def _save_aux_choice(
    ...
    *,
    ...
    """
    from rewired_cli.config import load_config, save_config
...
def _aux_config_menu() -> None:
    ...
    """
    from rewired_cli.config import load_config
...
def _aux_select_for_task(task: str) -> None:
    ...
    ``rewired model`` flow, then route aux tasks to them here.
    """
    from rewired_cli.config import load_config
    from rewired_cli.model_switch import list_authenticated_providers
...
def _aux_flow_provider_model(
    ...
    from rewired_cli.auth import _prompt_model_selection
    from rewired_cli.models import get_pricing_for_provider
...
def _prompt_provider_choice(choices, *, default=0):
    ...
    """
    try:
        from rewired_cli.setup import _curses_prompt_choice
...
def _model_flow_openrouter(config, current_model=""):
    ...
    from rewired_cli.models import model_ids, get_pricing_for_provider
...
def _model_flow_ai_gateway(config, current_model=""):
    ...
    from rewired_cli.models import ai_gateway_model_ids, get_pricing_for_provider
...
def _model_flow_nous(config, current_model="", args=None):
    ...
    from rewired_cli.nous_subscription import prompt_enable_tool_gateway
    ...
    from rewired_cli.models import (
    ...
    if free_tier and not model_ids:
        ...
        if unavailable_models:
            from rewired_cli.auth import DEFAULT_NOUS_PORTAL_URL
...
def _model_flow_openai_codex(config, current_model=""):
    ...
    from rewired_cli.codex_models import get_codex_model_ids
    ...
    if not _codex_token:
        try:
            from rewired_cli.auth import resolve_codex_runtime_credentials
...
def _model_flow_qwen_oauth(_config, current_model=""):
    ...
    from rewired_cli.models import fetch_api_models
...
def _model_flow_minimax_oauth(config, current_model="", args=None):
    ...
    from rewired_cli.models import _PROVIDER_MODELS
...
def _model_flow_custom(config):
    ...
    """
    from rewired_cli.auth import _save_model_choice, deactivate_provider
    from rewired_cli.config import get_env_value, load_config, save_config
    ...
    from rewired_cli.models import probe_api_models
    ...
    if model_name:
        ...
        print("Endpoint saved. Use `/model` in chat or `rewired model` to set a model.")
...
def _model_flow_azure_foundry(config, current_model=""):
    ...
    * Manual entry when every probe fails (private endpoints, etc.).
    ...
    """
    from rewired_cli.auth import _save_model_choice, deactivate_provider  # noqa: F401
    from rewired_cli.config import (
        get_env_value,
        save_env_value,
        load_config,
    ...
    from rewired_cli import azure_detect
    import getpass
...
def _remove_custom_provider(config):
    ...
    try:
        ...
        idx = menu.show()
        from rewired_cli.curses_ui import flush_stdin
...
def _model_flow_copilot(config, current_model=""):
    ...
    from rewired_cli.config import save_env_value, load_config, save_config
    from rewired_cli.models import (
        fetch_api_models,
    ...
    if not api_key:
        ...
        if choice == "1":
            try:
                from rewired_cli.copilot_auth import copilot_device_code_login
            ...
            # Validate token type
            try:
                from rewired_cli.copilot_auth import validate_copilot_token
...
def _model_flow_copilot_acp(config, current_model=""):
    ...
    try:
        ...
        print(f"  ⚠ {exc}")
        print(
            "  Set REWIRED_COPILOT_ACP_COMMAND or COPILOT_CLI_PATH if Copilot CLI is installed elsewhere."
        )
        return
...
def _prompt_api_key(pconfig, existing_key: str, provider_id: str = "") -> tuple:
    """Shared API-key entry point for ``rewired setup`` / ``rewired model``.
    ...
    recover from a malformed paste without editing ``~/.rewired/.env`` by hand.
    ...
    from rewired_cli.auth import LMSTUDIO_NOAUTH_PLACEHOLDER
    from rewired_cli.config import save_env_value
    ...
    if choice.startswith("c"):
        ...
        print(
            f"  API key cleared.  Re-run `rewired setup` to configure {pconfig.name} again."
        )
        return "", True
...
def _model_flow_api_key_provider(config, provider_id, current_model=""):
    ...
    if provider_id == "lmstudio":
        from rewired_cli.auth import AuthError
        from rewired_cli.models import fetch_lmstudio_models
        ...
        from rewired_cli.models import fetch_ollama_cloud_models
...
def _run_anthropic_oauth_flow(save_env_value):
    ...
    def _activate_claude_code_credentials_if_available() -> bool:
        ...
        if creds and (
            ...
            from rewired_constants import display_rewired_home as _dhh_fn
...
def _model_flow_anthropic(config, current_model=""):
    ...
    from rewired_cli.auth import get_anthropic_key
    ...
    if selected:
        ...
        # providers if the user switches without running 'rewired model'.
        cfg = load_config()
        model = cfg.get("model")
        if not isinstance(model, dict):
            model = {"default": model} if model else {}
...
def cmd_login(args):
    ...
    from rewired_cli.auth import login_command
...
def cmd_logout(args):
    ...
    from rewired_cli.auth import logout_command
...
def cmd_auth(args):
    ...
    from rewired_cli.auth_commands import auth_command
...
def cmd_status(args):
    ...
    from rewired_cli.status import show_status
...
def cmd_cron(args):
    """Cron job management."""
    from rewired_cli.cron import cron_command
...
def cmd_webhook(args):
    ...
    from rewired_cli.webhook import webhook_command
...
def cmd_slack(args):
    ...
    if sub == "manifest":
        from rewired_cli.slack_cli import slack_manifest_command
...
def cmd_kanban(args):
    ...
    from rewired_cli.kanban import kanban_command
...
def cmd_hooks(args):
    ...
    from rewired_cli.hooks import hooks_command
...
def cmd_doctor(args):
    ...
    from rewired_cli.doctor import run_doctor
...
def cmd_dump(args):
    ...
    from rewired_cli.dump import run_dump
...
def cmd_debug(args):
    ...
    from rewired_cli.debug import run_debug
...
def cmd_config(args):
    ...
    from rewired_cli.config import config_command
...
def cmd_backup(args):
    ...
    if getattr(args, "quick", False):
        from rewired_cli.backup import run_quick_backup

        run_quick_backup(args)
    else:
        from rewired_cli.backup import run_backup
...
def cmd_import(args):
    ...
    from rewired_cli.backup import run_import
...
def cmd_version(args):
    """Show version."""
    print(f"Rewired v{__version__} ({__release_date__})")
    ...
    try:
        from rewired_cli.banner import check_for_updates
        from rewired_cli.config import recommended_update_command
...
def cmd_uninstall(args):
    ...
    from rewired_cli.uninstall import run_uninstall
...
def _gateway_prompt(prompt_text: str, default: str = "", timeout: float = 300.0) -> str:
    ...
    Used by ``rewired update --gateway`` so interactive prompts (stash restore,
    config migration) are forwarded to the messenger instead of being silently
    skipped.
    """
    import json as _json
    import uuid as _uuid
    from rewired_constants import get_rewired_home

    home = get_rewired_home()
...
def _web_ui_build_needed(web_dir: Path) -> bool:
    ...
    Mirrors the staleness logic that was used by the old Ink TUI.
...
def _run_npm_install_deterministic(
    ...
    *,
    ...
    the working tree dirty and causes the next ``rewired update`` to stash the
    lockfile — repeatedly.
    """
...
def _find_stale_dashboard_pids() -> list[int]:
    ...
    forgotten.  When ``rewired update`` replaces files on disk, the running
...
def _print_curator_first_run_notice() -> None:
    """Print a short heads-up about the skill curator after `rewired update`.
...
def _print_curator_recent_run_notice() -> None:
    ...
    rename. ``rewired update`` is a high-attention surface — surface the
    most recent run's rename map here, once.

    Show-once: state stamps ``last_run_summary_shown_at`` after printing.
    Subsequent ``rewired update`` invocations skip the block until a newer
...
def _kill_stale_dashboard_processes(
    ...
    Called at the end of ``rewired update`` (default ``reason``) and also
...
def _update_via_zip(args):
    """Update Rewired by downloading a ZIP archive.
...
# =========================================================================
# Fork detection and upstream management for `rewired update`
# =========================================================================
...
def _should_skip_upstream_prompt() -> bool:
    ...
    return (get_rewired_home() / SKIP_UPSTREAM_PROMPT_FILE).exists()
...
def _mark_skip_upstream_prompt():
    ...
    try:
        ...
        (get_rewired_home() / SKIP_UPSTREAM_PROMPT_FILE).touch()
    except Exception:
        pass
...
def _invalidate_update_cache():
    ...
    ``rewired update``, every profile is now current.
    """
    homes = []
    # Default profile home (Docker-aware — uses /opt/data in Docker)
    from rewired_constants import get_default_rewired_root

    default_home = get_default_rewired_root()
...
def _run_install_with_heartbeat(
    ...
    *,
    ...
    know ``rewired update`` is still progressing even if pip/uv itself is silent.
    """
...
class _UpdateOutputStream:
    """Stream wrapper used during ``rewired update`` to survive terminal loss.

    Wraps the process's original stdout/stderr so that:

    * Every write is also mirrored to an append-only log file
      (``~/.rewired/logs/update.log``) that users can inspect after the
    ...
    * Writes to the original stream that fail with ``BrokenPipeError`` /
    ...
    ``_install_hangup_protection``, this makes ``rewired update`` safe to
...
def _install_hangup_protection(gateway_mode: bool = False):
    ...
    Users commonly run ``rewired update`` in an SSH session or a terminal
    ...
    2. ``sys.stdout`` / ``sys.stderr`` are wrapped to mirror output to
       ``~/.rewired/logs/update.log`` and to silently absorb
    ...
    **intentionally left alone** — those are legitimate cancellation
    ...
    In gateway mode (``rewired update --gateway``) the update is already
    ...
    try:
        ...
        # hermes_cli.config.get_rewired_home to simulate setup failure.
        from rewired_cli.config import get_rewired_home as _get_rewired_home

        logs_dir = _get_rewired_home() / "logs"
...
def _cmd_update_check():
    """Implement ``rewired update --check``: fetch and report without installing."""
...
def _ensure_fhs_path_guard() -> None:
    ...
    repaired on ``rewired update`` without requiring a reinstall.  The
    ...
    path_comment = (
        "# Rewired — ensure /usr/local/bin is on PATH " "(RHEL non-login shells)"
    )
    wrote_any = False
...
def _run_pre_update_backup(args) -> None:
    """Create a full zip backup of REWIRED_HOME before running the update.
    ...
    REWIRED_HOME directories.  The ``--backup`` flag on ``rewired update``
    ...
    try:
        from rewired_cli.backup import create_pre_update_backup
    except Exception as exc:
        print(
    ...
    # Render path using display_rewired_home so the user sees ~/.rewired/...
    try:
        from rewired_constants import get_rewired_home, display_rewired_home

        home = get_rewired_home()
        try:
            display_path = f"{display_rewired_home()}/{out_path.relative_to(home)}"
        except ValueError:
...
def cmd_update(args):
    """Update Rewired to the latest version.

    Thin wrapper around ``_cmd_update_impl``: installs hangup protection,
    runs the update, then restores stdio on the way out (even on
    ``sys.exit`` or unhandled exceptions).
    """
    from rewired_cli.config import is_managed, managed_error

    if is_managed():
        managed_error("update Rewired")
        return
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # the old bytecode (e.g. get_rewired_home added to hermes_constants).
        removed = _clear_bytecode_cache(PROJECT_ROOT)
        if removed:
            print(
                f"  ✓ Cleared {removed} stale __pycache__ director{'y' if removed == 1 else 'ies'}"
        ...
        # attributes like display_rewired_home() added since the last release.
        try:
            import importlib
            import rewired_constants as _hc
        ...
        # seed_profile_skills() uses subprocess with an explicit REWIRED_HOME so
        # it is not affected by sync_skills()'s module-level REWIRED_HOME cache,
        # which means the active profile is reliably synced regardless of whether
        # the caller's REWIRED_HOME env var points at the default or a named profile.
        try:
            from rewired_cli.profiles import (
                list_profiles,
                seed_profile_skills,
            )
        ...
        # When running as ``rewired update --gateway`` (spawned by the gateway's
        ...
        if gateway_mode:
            _exit_code_path = get_rewired_home() / ".update_exit_code"
            try:
        ...
        try:
            from rewired_cli.gateway import (
                is_macos,
                ...
                for scope, scope_cmd in [
                    ...
                    try:
                        ...
                        for line in result.stdout.strip().splitlines():
                            ...
                            # path in `rewired gateway restart`
            ...
            if restarted_services or killed_pids:
                ...
                if unmapped_count:
                    ...
                    print("    Restart manually: rewired gateway run")
                    if unmapped_count > 1:
                        print(
                            "    (or: rewired -p <profile> gateway run  for each profile)"
                        )
        ...
        # every `rewired update` surfaces the issue until the user migrates.
        try:
            from rewired_cli.gateway import (
            ...
            if supports_systemd_services() and has_legacy_hermes_units():
                ...
                print()
                print("    rewired gateway migrate-legacy")
                print()
        ...
        print("  rewired model              # Select provider and model")
...
def cmd_profile(args):
    ...
    from rewired_constants import display_rewired_home
    ...
    if action is None:
        ...
        for p in profiles:
            if p.name == profile_name or (profile_name == "default" and p.is_default):
                ...
                if p.alias_path:
                    print(f"Alias:          {p.name} → rewired -p {p.name}")
                break
        print()
        return
    ...
    if action == "list":
        ...
        try:
            ...
            if name == "default":
                print(f"Switched to: default (~/.rewired)")
            else:
        ...
        try:
            ...
            if not no_alias:
                ...
                if collision:
                    ...
                    print(f"  Or access via flag:     rewired -p {name} chat")
                else:
        ...
        from rewired_cli.profiles import profile_exists
        ...
        from rewired_cli.profiles import rename_profile
        ...
        from rewired_cli.profiles import export_profile
        ...
        from rewired_cli.profiles import import_profile
        ...
        import tempfile
        from rewired_cli.profile_distribution import (
            plan_install,
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"  Review them with:  rewired -p {plan.manifest.name} cron list"
                )
            print(f"\n  Use with:      rewired -p {plan.manifest.name} chat")
        ...
        from rewired_cli.profiles import get_profile_dir, normalize_profile_name
        ...
        try:
            ...
            if plan.has_cron:
                print(
                    ...
                    f"rewired -p {plan.manifest.name} cron list"
                )
        except (DistributionError, ValueError) as e:
            print(f"Error: {e}")
            sys.exit(1)
        ...
        from rewired_cli.profile_distribution import describe_distribution, DistributionError
...
def _render_distribution_plan(plan) -> None:
    ...
    from rewired_cli.profile_distribution import MANIFEST_FILENAME
    mf = plan.manifest
...
def cmd_dashboard(args):
    ...
    if getattr(args, "stop", False):
        ...
        if not pids:
            print("No hermes dashboard processes running.")
            sys.exit(0)
        # Reuse the same SIGTERM-grace-SIGKILL path used after `rewired update`.
    ...
    if "REWIRED_WEB_DIST" not in os.environ:
        if not _build_web_ui(PROJECT_ROOT / "web", fatal=True):
            sys.exit(1)

    from rewired_cli.web_server import start_server

    embedded_chat = not getattr(args, "classic", False) or os.environ.get("REWIRED_DASHBOARD_TUI") == "1"
    start_server(
        host=args.host,
        port=args.port,
...
def cmd_completion(args, parser=None):
    ...
    from rewired_cli.completion import generate_bash, generate_zsh, generate_fish
...
def cmd_logs(args):
    ...
    from rewired_cli.logs import tail_log, list_logs
...
def _build_provider_choices() -> list[str]:
    ...
    try:
        from rewired_cli.models import CANONICAL_PROVIDERS as _cp
...
def main():
    ...
    try:
        from rewired_cli.stdio import configure_windows_stdio
        configure_windows_stdio()
    except Exception:
        pass

    from rewired_cli._parser import build_top_level_parser
    ...
    # =========================================================================
    from rewired_cli.fallback_cmd import cmd_fallback
    ...
    fallback_subparsers.add_parser(
        "add",
        help="Pick a provider + model (same picker as `rewired model`) and append to the chain",
    )
    fallback_subparsers.add_parser(
        "remove",
        aliases=["rm"],
    ...
    setup_parser = subparsers.add_parser(
        ...
        description="Configure Rewired with an interactive wizard. "
        "Run a specific section: rewired setup model|tts|terminal|gateway|tools|agent",
    )
    ...
    setup_parser.add_argument(
        ...
        "compatibility — a bare 'rewired setup' now does this.",
    )
    setup_parser.add_argument(
        "--quick",
        action="store_true",
    ...
    slack_manifest.add_argument(
        ...
        "writes to $REWIRED_HOME/slack-manifest.json.",
    )
    slack_manifest.add_argument(
        "--name",
        default=None,
    ...
    auth_spotify.add_argument(
        "--client-id", help="Spotify app client_id (or set REWIRED_SPOTIFY_CLIENT_ID)"
    )
    auth_spotify.add_argument(
        "--redirect-uri",
    ...
    status_parser = subparsers.add_parser(
        ...
        description="Display status of Rewired components",
    )
    ...
    cron_create.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.rewired/scripts/. Default mode: "
    ...
    cron_edit.add_argument(
        "--script",
        help=(
            "Path to a script under ~/.rewired/scripts/. Pass empty string to clear. "
    ...
    # =========================================================================
    from rewired_cli.kanban import build_parser as _build_kanban_parser
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.rewired/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.rewired/shell-hooks-allowlist.json."
        ),
    )
    ...
    doctor_parser = subparsers.add_parser(
        ...
        description="Diagnose issues with Rewired setup",
    )
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Rewired. Use 'hermes debug share' to "
...
""",
    ...
    checkpoints_parser = subparsers.add_parser(
        "checkpoints",
        help="Inspect / prune / clear ~/.rewired/checkpoints/",
    ...
    from rewired_cli.checkpoints import register_cli as _register_checkpoints_cli
    ...
    config_parser = subparsers.add_parser(
        ...
        description="Manage Rewired configuration",
    )
    ...
    def cmd_pairing(args):
        from rewired_cli.pairing import pairing_command
    ...
    skills_reset = skills_subparsers.add_parser(
        ...
        description=(
            "Clear a bundled skill's entry from the sync manifest (~/.rewired/skills/.bundled_manifest) "
            "so future 'rewired update' runs stop marking it as user-modified. Pass --restore to also "
    ...
    def cmd_skills(args):
        ...
        if getattr(args, "skills_action", None) == "config":
            ...
            from rewired_cli.skills_config import skills_command as skills_config_command

            skills_config_command(args)
        else:
            from rewired_cli.skills_hub import skills_command
    ...
    def cmd_plugins(args):
        from rewired_cli.plugins_cmd import plugins_command
    ...
    # subcommand — ``hermes --help``, ``rewired version``, ``hermes logs``,
    ...
    if _plugin_cli_discovery_needed():
        try:
            ...
            from rewired_cli.plugins import discover_plugins, get_plugin_manager
    ...
    try:
        from rewired_cli.curator import register_cli as _register_curator_cli
    ...
    def cmd_memory(args):
        ...
        if sub == "off":
            ...
            mem_dir = get_rewired_home() / "memories"
            ...
            if not existing:
                print(
                    f"\n  Nothing to reset — no memory files found in {display_rewired_home()}/memories/\n"
                )
                return
            ...
            print(f"  Files were in: {display_rewired_home()}/memories/\n")
        else:
            from rewired_cli.memory_setup import memory_command
    ...
    def cmd_tools(args):
        ...
        if action in ("list", "disable", "enable"):
            from rewired_cli.tools_config import tools_disable_enable_command

            tools_disable_enable_command(args)
        else:
            _require_tty("tools")
            from rewired_cli.tools_config import tools_command
    ...
    def cmd_computer_use(args):
        ...
        if action == "install":
            from rewired_cli.tools_config import _run_post_setup
    ...
    def cmd_mcp(args):
        from rewired_cli.mcp_config import mcp_command
        ...
        if action == "list":
            ...
            if not args.yes:
                if not _confirm_prompt(
                    ...
                    print("Cancelled.")
                    return
            sessions_dir = get_rewired_home() / "sessions"
            ...
            from rewired_cli.relaunch import relaunch
    ...
    claw_migrate.add_argument(
        "--no-backup",
        action="store_true",
        help="Skip the pre-migration zip snapshot of ~/.rewired/ (by default a "
        "single restore-point archive is written to ~/.rewired/backups/ "
    ...
    def cmd_claw(args):
        from rewired_cli.claw import claw_command
    ...
    update_parser = subparsers.add_parser(
        "update",
        help="Update Rewired to the latest version",
    ...
    uninstall_parser = subparsers.add_parser(
        "uninstall",
        help="Uninstall Rewired",
        description="Remove Rewired from your system. Can keep configs/data for reinstall.",
    )
    ...
    acp_parser = subparsers.add_parser(
        "acp",
        help="Run Rewired as an ACP (Agent Client Protocol) server",
        description="Start Rewired in ACP mode for editor integration (VS Code, Zed, JetBrains)",
    )
    _add_accept_hooks_flag(acp_parser)

    def cmd_acp(args):
        """Launch Rewired as an ACP server."""
        try:
    ...
    profile_create.add_argument(
        "--no-skills",
        action="store_true",
        help="Create an empty profile with no bundled skills (opts out of `rewired update` skill sync)",
    )
    ...
    dashboard_parser = subparsers.add_parser(
        ...
        description="Launch the Rewired web dashboard for managing config, API keys, and sessions",
    )
    ...
    dashboard_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Expose the in-browser Chat tab (embedded `rewired --tui` via PTY/WebSocket). "
            "Alternatively set REWIRED_DASHBOARD_TUI=1."
        ),
    )
    ...
    # path `rewired update` uses to clean up stale dashboards.
    dashboard_parser.add_argument(
        "--stop",
        action="store_true",
...
""",
    ...
    from rewired_cli.config import get_container_exec_info
    ...
    if args.command in _AGENT_COMMANDS or (
        ...
        try:
            from rewired_cli.plugins import discover_plugins
    ...
    # =========================================================================
    # feed command — launch the re:wired hive feed interface
    # =========================================================================
    feed_parser = subparsers.add_parser(
        "feed",
        help="Launch the re:wired hive task feed interface",
        description="Arrow-key navigable task feed for the hierarchical AI swarm",
    )

    def cmd_feed(args):
        print("Note: 'hermes feed' is deprecated. Use the 'rewired' command directly (faster).", file=sys.stderr)
        from swarm.cli import run_cli
        run_cli()
    ...
    if getattr(args, "oneshot", None):
        from rewired_cli.oneshot import run_oneshot
